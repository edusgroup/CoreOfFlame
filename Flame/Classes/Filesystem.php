...
class filesystem
{
    ...
    public function andEndSlash($path)
    {
     ...
     *
     */
    public function dir2array($pDir, $pFileType = self::ALL, $pFilter = null)
    {
        if (!is_dir($pDir))
            return [];
        $return = [];
     ...
     * @return array
     */
    public function rDir2Arr($pDir, $pFilter = null)
    {
        $list = [];
        foreach (scandir($pDir) as $item) {
            ...
            $item = $pDir . "/" . $item;
            if (is_dir($item)) {
                $tmpList = $this->rDir2Arr($item, $pFilter);
    ...
    public function nsToPath($pNs)
    {
        return str_replace('\\', '/', $pNs);
        // func. nsToPath
    }

    /** @todo в utils перенести */
    public function formatBytes($pSize, $pPrecision = 2)
    {
        if ($pSize < 1024) {
            return $pSize . ' B';
     ...
     */
    public function getFirstObjectInFolder($p_dir, $p_file_type = self::ALL, $p_regex = '')
    {
        if (!is_dir($p_dir)) {
            return;
        }
     ...
     * Результат: jpg
     */
    public function getExt($pFilename)
    {
    ...
    public function getName($pFilename)
    {
        $list = explode('.', $pFilename);
    ...
    public function copy($pFileSource, $pPathDist, $pFileName)
    {
        if (!is_readable($pFileSource)) {
            throw new \Exception('Файла не существует ' . $pFileSource);
        }
        $this->mkdir($pPathDist);
        $pathDist = self::andEndSlash($pPathDist);
        return copy($pFileSource, $pathDist . $pFileName);
    }

    public function copyR($pPathSource, $pPathDist, $pBackGround = false)
    {
        $this->mkdir($pPathDist);
        $bg = $pBackGround ? ' &' : '';
    ...
    public function getMimeType($pFileName)
    {
    ...
    public function rmdirR($pPathSource, $pBackGround = false)
    {
        $bg = $pBackGround ? ' &' : '';
    ...
    public function rmdir($pDir)
    {
        ...
        if (!is_dir($dir) || is_link($dir)) {
            // Удаляем
            if (!unlink($dir)) {
                throw new \Exception('Не удалось удалить файл: ' . $dir);
            }
            return true;
        } // if
        ...
        foreach (scandir($dir) as $item) {
            ...
            if (!$this->rmdir($file)) {
                ...
                // Рекурсивно удаляем
                if (!$this->rmdir($file)) {
                    throw new \Exception('Не удалось удалить папку: ' . $file);
                } // if
            } // if
        } // foreach
        // Удаляем папку
     ...
     */
    public function printFile($pFilename)
    {
     ...
     */
    /* public function pregUnlink(string $pRegex, string $pDir) {
     ...
     */
    public function rUnlink($pPath, $pFileType = self::ALL, $pFilter = null)
    {
        // TODO: Задумать над rm -rf *file*
        $fileList = $this->dir2array($pPath, $pFileType, $pFilter);
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            $this->rUnlink($filename . '/', $pFileType, $pFilter);
        }
    }
     ...
     */
    public function unlink($pFile)
    {
        return is_readable($pFile) ? unlink($pFile) : false;
    }

    public function dir2tree($pDir)
    {
        ...
        $return = [];
        $pos = 0;
        $this->_rdir2tree($pDir, $return, $pos);
        return $return;
    }
    ...
    private function _rdir2tree($pDir, array &$pDirList, integer &$pParentId)
    {
        ...
        foreach (scandir($pDir) as $itemName) {
            ...
            if (is_file($file)) {
                ...
                $this->_rdir2tree($file, $pDirList, $parent);
     ...
     * @throws \Exception
     */
    public function mkdir($pDir, $pRules = 0777)
    {
        if (is_dir($pDir)) {
            return true;
        }
        ...
        throw new \Exception('Can\'t create folder ' . $pDir, 24);
        // func. mkdir
    }
    ...
    public function getFile1of2($p_path_1, $p_path_2, $p_filename)
    {
     ...
     * @param $pFilename
     */
    public function loadFileContent($pFilename)
    {
     ...
     * @param $pFilename
     */
    public function loadFileContentUnSerialize($pFilename)
    {
        if (!is_readable($pFilename)) {
            return null;
        }
    ...
    public function rename($pOldFile, $pNewFile)
    {
        if (is_file($pOldFile)) {
